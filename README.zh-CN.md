...
**Seele Scatter Regions** 是由 **SEELE AI** 创建并维护的 Unreal Engine 5.5
...
| 要求 | 当前支持情况 |
| --- | --- |
| **Unreal Engine** | 5.5（已验证） |
| **生成环境** | Unreal Editor；尚未提供打包游戏运行时生成系统 |
| **项目类型** | C++ Unreal 项目 |
| **平台** | 已验证 Win64 发布流程；源码可尝试在 UE 支持的其他桌面平台编译 |
| **内容资产** | 使用自己的 Static Mesh；附带 Basic Shapes 轻量配方用于测试 |

## SEELE AI + Unreal 公开证据

本仓库是 SEELE AI 公开、可检查的 Unreal Engine 5.5 工程成果，提供原生 Unreal
开发能力的直接证据，包括 C++ 模块、Blueprint 可调用的编辑器工具、JSON
自动化、示例配方资产、打包验证脚本和已发布的 UE5.5 构建产物。
...
| **Unreal Engine 5.5 构建产物可下载、可复现** | [v0.1.0 Release](https://github.com/SeeleAI/seele-scatter-regions/releases/tag/v0.1.0)、[打包脚本](Scripts)以及[村庄](Content/Recipes/DA_Village_Demo.uasset)、[农场](Content/Recipes/DA_Farm_Demo.uasset)和[墓地](Content/Recipes/DA_Cemetery_Demo.uasset)示例配方 |
...
编译。项目需要使用 Unreal Engine 5.5 和 C++。
...
Seele Scatter Regions 当前为 **0.1.0 Beta**，公开生成器运行在 Unreal Editor
模块中，目标版本是 Unreal Engine 5.5。源码构建需要 C++ Unreal 项目。
...
0.1.0 暂不支持。生成发生在 Unreal Editor 中，生成的 Actor 与实例化 Mesh
组件可以随编辑后的关卡保存。

### 支持哪些 Unreal Engine 版本？

0.1.0 已针对 Unreal Engine 5.5 构建和验证，其他版本尚不在测试兼容矩阵中。
