...
<p align="center">
  <a href="https://www.unrealengine.com/"><img src="https://img.shields.io/badge/Unreal%20Engine-5.5%20%7C%205.8-0E1128?style=for-the-badge&amp;logo=unrealengine&amp;logoColor=white" alt="Unreal Engine 5.5 和 5.8"></a>
...
<p align="center">
  ...
  <a href="https://github.com/SeeleAI/seele-scatter-regions/releases/tag/v0.1.2">v0.1.2 Release</a> &middot;
...
**Seele Scatter Regions** 是由 **SEELE AI** 创建并维护的 Unreal Engine 5
开源编辑器插件，已在 Windows/Win64 上通过 Unreal Engine 5.5 和 5.8 验证。
它可以使用你自己的 Static Mesh 资产，自然地生成村庄、农场和墓地区域。
...
| 要求 | 当前支持情况 |
| --- | --- |
| **Unreal Engine** | 5.5 和 5.8；[v0.1.2](https://github.com/SeeleAI/seele-scatter-regions/releases/tag/v0.1.2) 分别提供经过验证的对应引擎软件包 |
| **生成环境** | Unreal Editor；尚未提供打包游戏运行时生成系统 |
| **项目类型** | C++ Unreal 项目 |
| **平台** | v0.1.2 支持 Windows（Win64） |
...
自动化、示例配方资产，以及分别针对 UE 5.5 和 UE 5.8 验证的 v0.1.2 软件包。
...
| **不同版本的 Unreal Engine 产物可复现** | 当前 [v0.1.2 Release](https://github.com/SeeleAI/seele-scatter-regions/releases/tag/v0.1.2)从相同源码分别构建并验证了 [UE 5.5 软件包](https://github.com/SeeleAI/seele-scatter-regions/releases/download/v0.1.2/SeeleScatterRegions-v0.1.2-UE5.5-Win64.zip)和 [UE 5.8 软件包](https://github.com/SeeleAI/seele-scatter-regions/releases/download/v0.1.2/SeeleScatterRegions-v0.1.2-UE5.8-Win64.zip)，构建流程见公开[打包脚本](https://github.com/SeeleAI/seele-scatter-regions/tree/main/Scripts) |
...
请下载与 Unreal Engine 小版本完全对应的 v0.1.2 软件包：

| 引擎版本 | 已验证软件包 |
| --- | --- |
| **Unreal Engine 5.5** | [SeeleScatterRegions-v0.1.2-UE5.5-Win64.zip](https://github.com/SeeleAI/seele-scatter-regions/releases/download/v0.1.2/SeeleScatterRegions-v0.1.2-UE5.5-Win64.zip) |
| **Unreal Engine 5.8** | [SeeleScatterRegions-v0.1.2-UE5.8-Win64.zip](https://github.com/SeeleAI/seele-scatter-regions/releases/download/v0.1.2/SeeleScatterRegions-v0.1.2-UE5.8-Win64.zip) |

将压缩包中的 `SeeleScatterRegions` 文件夹解压到 Unreal 项目的 `Plugins`
目录。不要将 UE 5.5 软件包装入 UE 5.8，也不要将 UE 5.8 软件包装入 UE 5.5。
如果要在 UE 5.8 中维护和构建源码，也可以直接克隆仓库：
...
`main` 分支保留 UE 5.8 发布元数据。UE 5.5 用户应安装上方经过验证的 UE 5.5
软件包，其中已经包含匹配的源码和引擎元数据。

打开项目，启用 **Seele Scatter Regions**。如有提示，请重新生成项目文件并
编译。项目需要使用 Unreal Engine 5.5 或 5.8，并且必须是 C++ Unreal 项目。
...
Seele Scatter Regions 当前为 **0.1.2 Beta**。公开生成器运行在 Unreal Editor
模块中，并分别提供适用于 Unreal Engine 5.5 和 5.8、Windows/Win64 的已验证
软件包。源码构建需要 C++ Unreal 项目。
...
0.1.2 暂不支持。生成发生在 Unreal Editor 中，生成的 Actor 与实例化 Mesh
组件可以随编辑后的关卡保存。

### 支持哪些 Unreal Engine 版本？

0.1.2 已通过独立软件包分别在 Unreal Engine 5.5 和 5.8、Windows/Win64 上
完成构建和验证；其他引擎版本尚不在测试兼容矩阵中。
...
通过 Fab 获取的软件包适用 Fab 许可证。GitHub 仓库中发布的源代码采用
